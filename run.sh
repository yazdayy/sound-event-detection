...
DATASET_DIR="--dataset_dir=../../../../../storage/leey0204/fsd50k_audioset/audioset"
...
python pytorch/main.py inference_prob --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --holdout_fold=1 --model_type=$MODEL_TYPE --loss_type='clip_bce' --augmentation='mixup' --batch_size=32 --iteration=70000 --cuda

# ------ Optimize thresholds ------
# Optimize sound event detection thresholds
python utils/optimize_thresholds.py optimize_sed_thresholds  --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --filename='main' --holdout_fold=1 --model_type=$MODEL_TYPE --loss_type='clip_bce' --augmentation='mixup' --batch_size=32 --iteration=50000

# ------ Calculate metrics ------
# Calculate statistics with automatic threshold optimization
python utils/calculate_metrics.py calculate_metrics --dataset_dir=$DATASET_DIR --workspace=$WORKSPACE --filename='main' --holdout_fold=1 --model_type=$MODEL_TYPE --loss_type='clip_bce' --augmentation='mixup' --batch_size=32 --iteration=50000 --data_type='evaluate' --at_thresholds --sed_thresholds
