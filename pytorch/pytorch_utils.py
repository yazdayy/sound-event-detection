...
def do_mixup_timeshift(x, mixup_lambda, mean=0, std=90):
    """Mixup data.
    ...
    #print('AFTER TIMESHIFT:', output, '\n', output.size())
    
    output = output[0 :: 2].transpose(0, -1) * mixup_lambda[0 :: 2] + \
        output[1 :: 2].transpose(0, -1) * mixup_lambda[1 :: 2]
    output = output.transpose(0, -1)
    #print('AFTER MIXUP:', output, '\n', output.size())
