...
def train(args):
    ...
    for batch_data_dict in train_loader:
        ...
        # Evaluate
        if (iteration % 1 == 0 and iteration > resume_iteration):# or (iteration == 0):
...
#            logging.info('Model saved to {}'.format(checkpoint_path))
        ...
        if 'mixup' in augmentation:
            if 'timeshift' not in augmentation:
                ...
                batch_output_dict = model(batch_data_dict['waveform'], mixup_lambda=batch_data_dict['mixup_lambda'], timeshift=True)
                batch_target_dict = {'target': do_mixup(batch_data_dict['target'], batch_data_dict['mixup_lambda'])}
        else:
